...
class TailFile extends Readable {
  constructor(filename, opts) {
    ...
    if (emitPositionWithChunk !== null && emitPositionWithChunk !== undefined) {
      if (typeof emitPositionWithChunk !== 'boolean') {
        const err = new RangeError('emitPositionWithChunk must be a boolean')
        err.code = 'ESENDPOS'
        err.meta = {
          got: emitPositionWithChunk
        }
        throw err
      }
    }
    ...
    this[kInode] = null
    this[kSendPos] = !!emitPositionWithChunk;
  }
  ...
  async _readRemainderFromFileHandle() {
    ...
    this.push(this[kSendPos] ? { chunk: buffer, positoin: this[kStartPos] } : buffer)
  ...
  async _readChunks(stream) {
    ...
    for await (const chunk of iterator) {
      const toPush = this.push(this[kSendPos] ? { chunk, positoin: this[kStartPos] } : chunk)
